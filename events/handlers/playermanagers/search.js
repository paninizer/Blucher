...
async function search(client, message, args, type, slashCommand) {
  ...
  try {
    ...
    var theresults = res.tracks
      .slice(0, max)
    var results = theresults.map((track, index) => `**${++index})** [\`${String(track.title).substring(0, 60).split("[").join("{").split("]").join("}")}\`](${track.url}) - \`${format(track.duration).split(" | ")[0]}\``)
      .join('\n');
    let toreact;
    if(slashCommand)
    ...
    var pickedresults = theresults.map((track, ii) => `${index == ii ? "" : "~~"}**${++ii})** [\`${String(track.title).substring(0, 60).split("[").join("{").split("]").join("}")}\`](${track.url}) - \`${format(track.duration).split(" | ")[0]}\`${index == ii ? "" : "~~"}`)
      .join('\n');
