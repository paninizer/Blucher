...
module.exports = {
	...
	run: async (client, message, args, cmduser, text, prefix, MusicPlayer, es, ls, GuildSettings) => {
		...
		if (!channel) {return message.reply({embeds: [new MessageEmbed().setTitle('<a:animated_wrong:947340139359789106> You need to join a voice channel').setColor(es.wrongcolor).setFooter(client.getFooter(es))]});}
		...
		if (botchannel) {return message.reply({embeds: [new MessageEmbed().setTitle(`<a:animated_wrong:947340139359789106> I am already connected in: \`${botchannel.name}\``).setFooter(client.getFooter(es))]});}
