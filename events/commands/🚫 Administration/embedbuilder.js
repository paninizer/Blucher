...
let buttonEvent = async (interaction) => {
  ...
  if (interaction?.message.id === msg.id) {
    if (interaction?.user.id === message.member.id) {
      ...
      await interaction?.reply({content : `<a:animated_wrong:947340139359789106> You are not allowed to do that! Only: <@${message.author?.id}>`, ephemeral : true}); //ephemeral message
    }
  }
}
