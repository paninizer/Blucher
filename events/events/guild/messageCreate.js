...
module.exports = async (client, message) => {
  try {
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`<a:animated_wrong:947340139359789106> **I am missing the Permission to EMBED LINKS (Sending Embeds)**`).catch(console.error)
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`<a:animated_wrong:947340139359789106> **I am missing the Permission to ADD REACTIONS**`).catch(console.error)
    ...
    if (command && !customcmd) {
      ...
      if(musicData && musicData.channel && musicData.channel == message.channel.id){
        return message.reply("<a:animated_wrong:947340139359789106> **Please use a command somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(console.error)
      }
...
//Improved by paninizer
//Original by paninizer
